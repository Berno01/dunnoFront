...
export class DashboardComponent implements OnInit {
  ...
  exportarExcelAgrupado() {
    ...
    this.dashboardService.getVentasExport(filters).subscribe({
      next: async (rows) => {
        ...
        try {
          await this.generarExcelAgrupado(rows);
        } catch (err) {
  ...
  exportarExcelDetallado() {
    ...
    this.dashboardService.getVentasExport(filters).subscribe({
      next: async (rows) => {
        ...
        try {
          await this.generarExcelDetallado(rows);
        } catch (err) {
          console.error('Error generando Excel', err);
          alert('Ocurrio un error al generar el archivo Excel.');
        }
  ...
  private async generarExcelAgrupado(rows: VentaExportRow[]) {
  ...
  private async generarExcelDetallado(rows: VentaExportRow[]) {
    const workbook = new ExcelJS.Workbook();
    ...
    a.href = url;
    a.download = 'ventas_detalle_' + fechaFile + '.xlsx';
    a.click();
    URL.revokeObjectURL(url);
  }
}
